    ...
    */
    ...
    for (auto vertex_handle : halfedge_mesh->vertices()) {
        ...
        for (auto halfedge_handle : vertex_handle.outgoing_halfedges()) {
            ...
            // printf(
            //     "from = %d, to = %d, face = %d, faceopp = %d, id1 = %d, id2 = %d, id3 = %d, id4 =
            //     "
            //     "%d\n",
            //     from,
            //     to,
...
static void node_hybrid_exec(ExeParams params)
{
    ...
    while (iteration_times-- > 0) {
        ...
        for (auto face_handle : halfedge_mesh->faces()) {
            ...
            const auto u3 = u[halfedge_handle.next().next().to().idx()];
            const double C_1 =
                cot_theta[halfedge_handle.next().idx()] * (x1 - x2).dot(x1 - x2) +
                cot_theta[halfedge_handle.next().next().idx()] * (x2 - x3).dot(x2 - x3) +
                cot_theta[halfedge_handle.idx()] * (x3 - x1).dot(x3 - x1);
            const double C_2 =
                cot_theta[halfedge_handle.next().idx()] * (u1 - u2).dot(x1 - x2) +
                cot_theta[halfedge_handle.next().next().idx()] * (u2 - u3).dot(x2 - x3) +
                cot_theta[halfedge_handle.idx()] * (u3 - u1).dot(x3 - x1);
            const double C_3 = cot_theta[halfedge_handle.next().idx()] *
                                   ((u1 - u2).x() * (x1 - x2).y() - (u1 - u2).y() * (x1 - x2).x()) +
                               cot_theta[halfedge_handle.next().next().idx()] *
                                   ((u2 - u3).x() * (x2 - x3).y() - (u2 - u3).y() * (x2 - x3).x()) +
                               cot_theta[halfedge_handle.idx()] *
                                   ((u3 - u1).x() * (x3 - x1).y() - (u3 - u1).y() * (x3 - x1).x());
            ...
            for (auto a_ : as) {
                for (auto b_ : bs) {
                    double energy =
                        cot_theta[halfedge_handle.next().idx()] *
                            (std::pow((u1 - u2).x() - a_ * (x1 - x2).x() - b_ * (x1 - x2).y(), 2) +
                             std::pow((u1 - u2).y() + b_ * (x1 - x2).x() - a_ * (x1 - x2).y(), 2)) +
                        cot_theta[halfedge_handle.next().next().idx()] *
                            (std::pow((u2 - u3).x() - a_ * (x2 - x3).x() - b_ * (x2 - x3).y(), 2) +
                             std::pow((u2 - u3).y() + b_ * (x2 - x3).x() - a_ * (x2 - x3).y(), 2)) +
                        cot_theta[halfedge_handle.idx()] *
                            (std::pow((u3 - u1).x() - a_ * (x3 - x1).x() - b_ * (x3 - x1).y(), 2) +
                             std::pow((u3 - u1).y() + b_ * (x3 - x1).x() - a_ * (x3 - x1).y(), 2)) +
                        lambda * std::pow((a_ * a_ + b_ * b_ - 1), 2);
            ...
            L[face_handle.idx()](0, 0) = a;
            L[face_handle.idx()](0, 1) = b;
            L[face_handle.idx()](1, 0) = -b;
            L[face_handle.idx()](1, 1) = a;
            // printf("hybrid: L_%d = {%f, %f, %f, %f}\n", face_handle.idx(), a, b, -b, a);
        }
